...
import { Search, ChevronRight, Check, X, Clock, CheckCheck } from 'lucide-react';
        ...
        <table className="w-full text-left text-xs border-collapse">
          ...
          <tbody className="divide-y divide-gray-100 font-medium">
            {filteredMembers.map((m) => {
              ...
              const adhkarDone = rec.habits.morningAdhkar && rec.habits.eveningAdhkar;

              return (
                <tr key={m.user.id} className="hover:bg-gray-50/80 transition-colors group">
                  <td className="py-3.5 px-4">
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-gray-400 font-medium">{m.user.email}</p>
                      </div>
                    </Link>
                  </td>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-12 bg-gray-100 rounded-full h-1.5">
                        <div
                          ...
                          style={{ width: `${m.todayProgress}%` }}
                        />
                      </div>
                      <span>{m.todayProgress}%</span>
                    </div>
                  </td>

                  <td className="py-3.5 px-3">{renderStatusSymbol(rec.prayers.fajr)}</td>
                  <td className="py-3.5 px-3">{renderStatusSymbol(rec.prayers.dhuhr)}</td>
                  <td className="py-3.5 px-3">{renderStatusSymbol(rec.prayers.asr)}</td>
                  <td className="py-3.5 px-3">{renderStatusSymbol(rec.prayers.maghrib)}</td>
                  <td className="py-3.5 px-3">{renderStatusSymbol(rec.prayers.isha)}</td>
                  ...
                  <td className="py-3.5 px-3 text-center">
                    ...
                    ) : rec.habits.morningAdhkar || rec.habits.eveningAdhkar ? (
